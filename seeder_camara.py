...
def salvar_no_supabase(lista_politicos):
    print("\n Iniciando Inserção REAL no Banco de Dados...")

    try:
        # O Supabase aceita inserir uma lista inteira de uma vez (Bulk Insert)
        resposta = supabase.table("politicos").insert(lista_politicos).execute()
        print(
            f"✅ Sucesso! {len(resposta.data)} políticos foram inseridos no Supabase."
        )
    except Exception as e:
